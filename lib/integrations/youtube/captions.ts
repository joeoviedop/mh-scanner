...
export type CaptionResult = {
  ...
  fullText: string;
  wordCount: number;
  source: "watch:ttml" | "watch:srv3" | "watch:vtt" | "timedtext" | "apify";
};

export type CaptionTrackDebug = {
  languageCode?: string;
  kind?: string;
  name?: string;
  isAutoGenerated: boolean;
...
export type CaptionFetchDebug = {
  watchTracks: CaptionTrackDebug[];
  fallbackLanguages: string[];
  timedTextAttempts: TimedTextAttempt[];
  lastSource?: CaptionResult["source"];
  apify?: {
    runId: string;
    datasetId?: string;
    itemCount: number;
...
async function fetchTimedText(params: {
  ...
  for (const lang of languageVariants) {
    ...
    if (params.isAutoGenerated) {
      ...
      addCandidate({ ...base, tlang: "es" });
      addCandidate({ ...base, kind: "asr", tlang: "es" });
      addCandidate({ ...base, tlang: "es-419" });
      addCandidate({ ...base, kind: "asr", tlang: "es-419" });
...
export async function fetchCaptions(videoId: string): Promise<CaptionFetchOutcome> {
  ...
  const debug: CaptionFetchDebug = {
    watchTracks: watchTracks.map((track) => ({
  ...
  debug.fallbackLanguages = Array.from(fallbackLanguages);
  ...
  for (const { track } of rankedTracks) {
    ...
    if (baseResult.segments.length > 0 && baseResult.source) {
      debug.lastSource = baseResult.source;
      return {
        result: buildResult(videoId, track, baseResult.segments, baseResult.source),
        debug,
      };
    }
    ...
    debug.timedTextAttempts.push(...timedText.attempts);

    if (timedText.text) {
      const segments = parseTtml(timedText.text);
      if (segments.length > 0) {
        debug.lastSource = "timedtext";
        return {
          result: buildResult(videoId, track, segments, "timedtext"),
          debug,
        };
      }
    }
  ...
  for (const lang of fallbackLanguages) {
    ...
    if (timedText.text) {
      let segments = parseTtml(timedText.text);
      if (segments.length === 0) {
        segments = parseSrv3(timedText.text);
      }
      if (segments.length > 0) {
        ...
        debug.lastSource = "timedtext";
        return {
          result: buildResult(videoId, syntheticTrack, segments, "timedtext"),
          debug,
        };
      }
    }
  ...
  try {
    ...
    if (apifyResult) {
      debug.lastSource = "apify";
      debug.apify = apifyResult.debug;
      ...
      const syntheticTrack: WatchCaptionTrack = {
        ...
        kind: "asr",
        isAutoGenerated: true,
        name: "Apify Transcript",
      };

      return {
        result: buildResult(videoId, syntheticTrack, apifyResult.segments, "apify"),
        debug,
      };
    }
  } catch (error) {
    if (process.env.NODE_ENV !== "production") {
      console.warn("[captions] apify fallback failed", error);
    }
    debug.apify = {
      runId: "error",
  ...
  if (process.env.NODE_ENV !== "production") {
    console.warn("[captions] no public subtitles", {
      videoId,
      watchTracks: debug.watchTracks,
      fallbackLanguages: debug.fallbackLanguages,
      timedTextAttempts: debug.timedTextAttempts.slice(-5),
      apify: debug.apify,
    });
  }
