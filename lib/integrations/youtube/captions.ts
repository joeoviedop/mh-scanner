...
async function fetchTimedText(params: {
  ...
  if (params.language) {
    const [baseLanguage] = params.language.split(/[-_]/);
    url.searchParams.set("lang", baseLanguage);
  }

  if (params.isAutoGenerated) {
    url.searchParams.set("kind", "asr");
  } else if (params.trackName) {
    url.searchParams.set("name", params.trackName);
  }
  ...
  if (!response.ok) {
    throw new YouTubeCaptionsError(`Timed text download failed (${response.status})`, response.status);
  }
...
export async function fetchCaptions(videoId: string, apiKey: string): Promise<CaptionResult | null> {
  ...
  try {
    ...
    if (!(error instanceof YouTubeCaptionsError && error.status === 404)) {
      throw error;
    }
  }
